...
def vessel_get_query(user_question):
    llm_openai = ChatOpenAI(openai_api_key=ConfigData.OPEN_AI_KEY, model='gpt-3.5-turbo-0125')#, temperature=0)
    # llm_openai = ChatOpenAI(openai_api_key=ConfigData.OPEN_AI_KEY, model='gpt-4-0125-preview', temperature=0)

    table_schema = ConfigData.TABLE_SCHEMA_VESSEL
    schema_description = ConfigData.SCHEMA_DESCRIPTION_VESSEL
    json_ex_1 = ConfigData.FEW_SHOT_EXAMPLE_1_VESSEL
...
def vess_get_final_out(question):
    client = pymongo.MongoClient(ConfigData.MONGO_DB_URI)
    db = client[ConfigData.DB_NAME]
    collection_name = db[ConfigData.COLLECTION_NAME_VESSEL]

    query_1 = vessel_get_query(user_question=question)
